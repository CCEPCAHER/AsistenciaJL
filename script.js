...
function generateSchedule() {
  ...
  weeks.forEach(week => {
    ...
    turnos.forEach(turno => {
      ...
      if (fixedAssignments[key]) {
        ...
        if (f.capitan) {
          shift.capitan = f.capitan; assignedCounts[f.capitan]++; inShift[f.capitan]=true;
          if (!allowMultiple) weekly[week][f.capitan] = true;
        }
        f.publicadores.forEach((pp,i) => {
          if (pp) {
            shift.publicadores[i] = pp; assignedCounts[pp]++; inShift[pp]=true;
            if (!allowMultiple) weekly[week][pp] = true;
          }
        });
      }

      let candidates = availability[key] ? [...availability[key]] : persons.map(p=>p.name);
      const filterFn = name => !inShift[name] && (allowMultiple || !weekly[week][name]);
      ...
      if (!shift.capitan) {
        ...
        if (chosen) {
          shift.capitan = chosen; assignedCounts[chosen]++; inShift[chosen]=true;
      ...
      for (let i=0; i<3; i++) {
        if (!shift.publicadores[i]) {
          ...
          if (chosen) {
            shift.publicadores[i] = chosen; assignedCounts[chosen]++; inShift[chosen]=true;
            if (!allowMultiple) weekly[week][chosen] = true;
            candidates = candidates.filter(n=>n!==chosen);
  ...
  if (turnos15) {
    const extra = schedule.map(s=>({ week: s.week==='S1'?'S3':'S4', turno: s.turno, capitan: s.capitan, publicadores: [...s.publicadores] }));
...
const turnoMapping = { MM:'day-lunes', MT1:'day-martes-1', MT2:'day-martes-2', XT1:'day-miercoles-1', XT2:'day-miercoles-2', J:'day-jueves', VT1:'day-viernes-1', VT2:'day-viernes-2', S:'day-sabado', D:'day-domingo' };
...
function displaySchedule(data) {
  ...
  ['Semana','Turno','Capitán','Publicador 1','Publicador 2','Publicador 3'].forEach(text=>{
    const th = document.createElement('th'); th.textContent = text; hr.appendChild(th);
  });
  ...
  data.forEach(item=>{
    ...
    ['week','turno','capitan'].forEach((key,i)=>{
      ...
      cell.textContent = key==='week'?item.week:(key==='turno'?item.turno:item.capitan||'Vacante');
    ...
    item.publicadores.forEach(pub=>{
      const cell = row.insertCell(); cell.textContent = pub||'Vacante'; if (dayClass) cell.classList.add(dayClass);
    });
  });
...
function addPDFButton() {
  ...
  btn.id = 'btnExportPDF'; btn.textContent = 'Exportar PDF';
...
function exportToPDF() {
  if (typeof window.jspdf==='undefined'||typeof window.jspdf.jsPDF==='undefined') { alert('jsPDF no cargado.'); return; }
  if (typeof window.jspdf.jsPDF.autoTable!=='function') { alert('autoTable no cargado.'); return; }
  const { jsPDF } = window.jspdf;
  const doc = new jsPDF({ orientation:'portrait', unit:'pt', format:'a4' });
  doc.setFontSize(18); doc.text('Calendario de Turnos', 40, 50);
  doc.autoTable({ html:'.schedule-table', startY:70, theme:'grid', headStyles:{ fillColor:[22,160,133], textColor:[255,255,255], halign:'center'}, styles:{ fontSize:9, cellPadding:5}, columnStyles:{ 0:{halign:'center'},1:{halign:'center'}}, didParseCell(data){ const el=data.cell.raw; if(el&&el.classList){ let color=null; Object.entries(weekMapping).forEach(([k,cls])=>{ if(el.classList.contains(cls)) color=getComputedStyle(el).backgroundColor }); Object.entries(turnoMapping).forEach(([k,cls])=>{ if(el.classList.contains(cls)) color=getComputedStyle(el).backgroundColor }); if(color&&color!=='transparent'){ const rgb=color.match(/\d+/g); if(rgb&&rgb.length>=3) data.cell.styles.fillColor=rgb.slice(0,3).map(n=>parseInt(n)) } } } });
...
function analyzeAssignments(data) {
  ...
  data.forEach(s=>{ if(s.capitan&&counts[s.capitan]>=0) counts[s.capitan]++; s.publicadores.forEach(pub=>{ if(pub&&counts[pub]>=0) counts[pub]++; }); });
  let cont = document.getElementById('assignmentAnalysis');
  if(!cont){ cont = document.createElement('div'); cont.id='assignmentAnalysis'; cont.style.marginTop='20px'; const pdfBtn=document.getElementById('btnExportPDF'); if(pdfBtn) pdfBtn.parentNode.insertBefore(cont,pdfBtn.nextSibling); else scheduleTableContainer.parentNode.insertBefore(cont,scheduleTableContainer.nextSibling); }
  cont.innerHTML='';
  const h3=document.createElement('h3'); h3.textContent='Resumen de Asignaciones Generadas'; cont.appendChild(h3);
  const ul=document.createElement('ul'); ul.style.listStyle='none'; ul.style.paddingLeft='0';
  persons.slice().sort((a,b)=>a.name.localeCompare(b.name)).forEach(p=>{ const li=document.createElement('li'); li.textContent=`${p.name}: ${counts[p.name]} turno(s)`; li.style.color=counts[p.name]===0?'#e74c3c':'#27ae60'; li.style.marginBottom='3px'; ul.appendChild(li); });
  cont.appendChild(ul);
}
